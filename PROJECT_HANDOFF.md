...
- 目标：从“练手闭环”升级为**垂直人群主动搜索、重复使用**的无压力习惯打卡；前期 60-200 用户，传播往后放。
- 主攻 v1.2 核心体验升级（见下），订阅消息模板、分享海报已排期稍后跟进。
- 冷启动 = 自动化脚本生成素材 + 人工发布结合，不做全自动发布。

## 待办

1. P0：线上验证 v1.1.0：微信搜索名称进入 → 管理页确认显示 `习惯打卡 v1.1.0`，无“开发版/体验版”角标。
2. P0：v1.2 核心体验升级（垂直人群功能+体验+设定），方向见 `UI优化方案-v1.md` 与产品方向节。
