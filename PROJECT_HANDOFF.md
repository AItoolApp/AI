...
- **微信认证进行中**：平台通知 2026-09-17 前完成；认证已付款，等待审核。后台“通过名称搜索此小程序”当前为“无法被搜索”，违规记录已确认无记录，应为未认证导致的搜索限制，认证通过后应恢复。**预计 2026-08-19 前后可能有认证审核电话，需留意接听。**
...
- 本地 `main` 与 `origin/main` 已同步（2026-08-18 push：`42d6eab` v1.1 数据加固 + `d0d63a9` v1.1.0 提审文档）。
...
1. P0：微信认证：预计 2026-08-19 前后有认证审核电话，注意接听；认证通过后验证名称搜索恢复（限期 2026-09-17）。
2. P0：跟进 v1.1.0 审核进度；审核通过后立即发布（线上 1.0.0 为旧代码，存在全量读写存储风险）。
3. P0：查看并回填小程序备案号（微信暂无通知，等后台可查后处理）；拿到号后在管理页「关于」区域展示并更新本文件。
4. P0：公安联网备案：`beian.gov.cn` 提交小程序类型公安备案（备案通过后 30 日内完成）；与网站公安备案分开办理。
5. P1：用户提供订阅消息模板 ID；拿到后替换 `dailyReminder/index.js` 中的 `YOUR_TEMPLATE_ID` 并部署云函数。
6. P2（v1.2）：按 `UI优化方案-v1.md` 推进交互升级（P1-1~P1-5：打卡按钮可访问性、打卡彩蛋、删除撤销、日历详情、主题引导）。
7. P2：按上线前准备清单执行体验版自测、运营素材、数据埋点。
